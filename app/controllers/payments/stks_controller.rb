...
module Payments
  class StksController < PaymentsController
    ...
    def stk_params
      params.require(:payments_stk).permit(:till_number, :first_name, :last_name, :phone, :email, :currency, :value, :callback_url)
    end

    def stk_request
      {
        payment_channel: "M-PESA",
        till_number: stk_params[:till_number],
        ...
        callback_url: stk_params[:callback_url],
      }
    end
    ...
    def stk_push
      set_stk_push
      if @k2_stk
        @k2_stk.send_stk_request(stk_request)
